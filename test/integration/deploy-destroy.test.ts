...
function run(cmd: string, cwd = SAMPLE_APP) {
  execSync(cmd, { cwd, stdio: "inherit", env: { ...process.env, INTEG_STACK_NAME: STACK_NAME } });
}
...
describeInteg("CDK deploy -> test -> destroy", () => {
  ...
  beforeAll(async () => {
    run("bun install");
    run(`bunx cdk deploy ${STACK_NAME} --require-approval never --outputs-file outputs.json`);
  ...
  afterAll(async () => {
    try {
      ...
      console.warn(`Stack destroy failed — run 'bun run integ:cleanup' to clean up ${STACK_NAME}`);
    }
  }, 15 * 60 * 1000);
